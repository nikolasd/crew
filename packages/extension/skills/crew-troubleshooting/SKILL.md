...
- **Profile-first worker provisioning:** Reserved adapters (claude, codex, copilot, ompRpc) require profiles. Use `crew_profile { adapter, model, startupOptions: { <adapter>: { mode: "tui" } } }` (startup options are tagged by adapter kind, e.g. `{ claude: { mode: "tui" } }`) first, then `crew_worker { profileId }`, then `crew_run`. Omitting `mode` selects `"tui"` — for claude, `"protocol"` is also a legal value (experimental, under evaluation, not yet recommended for use) and is never selected unless named explicitly. See the crew-orchestration skill for the full flow.
...
- A claude, codex, or copilot worker that hits its own first-run trust dialog stops there, in `starting`, before ever reaching a delivered prompt — never `waitingUser`, since no turn has happened yet to settle. It raises an `EscalationRaised { reason: "vendorFirstRunGate" }`, delivered as a digest into your own session the same way a `workerQuestion` is, with a runtime-authored question naming the vendor and the gate ("The Codex CLI is waiting on its first-run directory-trust prompt...", for example), telling you to answer it in the worker's pane or cancel the run, and — for a gate with a one-time config fix — how to avoid it next time. The pane behind it holds the real, unanswered dialog; crew never presses Enter into it or reads it for you. Use `/crew reopen <runId>` to attach and answer it by hand like any TUI prompt — the readiness check that is waiting on it notices on its own once you do, and the run proceeds to its first delivered prompt exactly as if the dialog had never appeared; or `crew_stop` to cancel the run instead of answering. To avoid hitting it again on this repository, trust it once with that vendor outside crew, or add its documented config entry — see `docs/compatibility.md`'s "TUI First-Run Gate Detection" table.
- A parked (`waitingUser`) omp worker is never a first-run trust dialog. Unlike claude, codex, and copilot, omp has no first-run `GateKind`, so it never reaches a parked run over this at all: a misconfigured machine instead fails the `run/submit` (or `run/retry`) call itself, which returns an error directly rather than parking, with the run forced to `failed` if the start error left it non-terminal (see `docs/compatibility.md`'s "TUI First-Run Gate Detection", including the `startup.quiet` prerequisite). Never a silent no-op, and never a run that quietly goes red later with no error in hand. So a park on an omp worker mid-task is a genuine `workerQuestion`, not a trust decision in disguise — and it arrives as a digest pushed into your own session, the leader's, never straight to the human. Read the digest and decide: answer it yourself via `crew_send` if the run's own context is enough, or escalate to the user only when the decision is genuinely theirs to make.
...
- A claude worker started with `mode: "protocol"` (experimental, under evaluation, not yet recommended for use) requires the target repository to already have its one-time trust prompt accepted in claude, in every mode, before crew will launch a worker there — crew reads that record and never writes it. If it hasn't been, the run is refused before any vendor process spawns, with a typed escalation naming the remedy: run claude once in this repository yourself and accept the prompt, then resubmit. Per claude's own documentation, this trust is keyed on the git repository root, and in a worktree it uses the main checkout's root — trusting a parent folder, or a worktree path specifically, does not count.
