...
1. **Resume first.** For each stuck run, the daemon tries to continue it on the *same* vendor session its previous incarnation established (never a retry, never a new run — a genuine continuation with the same run id). Eligibility requires a real vendor session id, a resolvable adapter, TUI mode, and its deterministic transcript file actually existing on disk. Headless mode can't resume (retired); the claude protocol adapter can't resume either, with its own distinct refusal (experimental, under evaluation, resume is out of scope for its first version).
