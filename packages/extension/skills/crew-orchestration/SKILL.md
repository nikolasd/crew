...
1. **Register a profile** — `crew_profile { adapter: "claude", startupOptions: { claude: { mode: "tui" } } }`. The profile captures adapter, model, startup options, and environment allowlist. Receives a `profileId`. `model` is optional and, until one is configured for that adapter, is never a decision you make on the leader's behalf: omit it (or pass a suggestion, e.g. "sonnet") and, in an interactive session, crew opens the model-selection dialog itself and asks the human — your suggestion is only preselected, not accepted. That answer is persisted into the repo's `.omp/crew.json` and reused silently on every later call, for that adapter, for good; once it's set, this parameter is only ever compared to it. A typed `model-not-configured` error means there was no way to ask (no interactive UI, the dialog timed out, or nothing to offer). Passing a model that *conflicts* with one already configured is refused with a typed `model-conflict` error naming the stored value — `crew_profile` never overwrites it and never silently ignores the conflict; the fix is editing the repository's `.omp/crew.json` directly (`/crew config path` locates it — `/crew config` has no set/edit subcommand). `mode: "tui"` is filled in automatically for a reserved adapter (claude, codex, copilot, ompRpc) when omitted — headless is retired, so this is the only silent default today. For claude specifically, `mode: "protocol"` is also a legal value — it drives the vendor over its own streaming-JSON protocol instead of a terminal, and it is never selected unless you name it explicitly. It is experimental, under evaluation, and not yet recommended for use.
...
1. A finished turn parks the run at `waitingUser` and journals the turn-end — a TUI worker, which stays alive between turns, sits there. A protocol worker's own process exits when its turn ends, so this park is transient: the exit typically lands within milliseconds, and the run you actually observe commonly already reads terminal (`cancelled`, ADR-0027's `unrendered_verdict()` — real work done, no `run/finish` call ever rendered a verdict on it). Either way, the `/crew` monitor emits a milestone for the settle the same way it does for terminal states, worker questions, and timeouts — you do not have to poll for it.
2. `crew_run { op: "result", runId }` is readable as soon as the turn settles, whether the run is still parked at `waitingUser` (TUI, or a protocol run you caught in time) or has already gone terminal (`cancelled`, the common protocol case) — no need to cancel the run first to read its answer either way.
3. From there:
   - `crew_send { runId, text, kind }` to follow up **while the worker's own process is still alive** — a follow-up on a live run is never refused, so there is no need to cancel-and-resubmit just to keep steering that conversation. Once a protocol worker's process has exited (the run already reads terminal), there is no live adapter left to steer: the message still journals, but nothing is driving it — continuing the work means a fresh `crew_run { op: "submit" }`, not a follow-up.
   - `crew_run { op: "finish", runId, outcome: "succeeded" | "failed" }` — the leader's own decision that this conversation is done. States the outcome explicitly (default `"succeeded"`); never inferred from the vendor's own turn markers, because a finished turn only means "the vendor stopped talking," not "the task went well." Works on any non-terminal run, not only one that has settled a turn — which is exactly why it refuses once the run is already terminal (the protocol case above, `invalid_params`): there is no verdict left to render, and `op: "result"` is the whole of what you get.
   - `crew_stop { runId, outcome: "done" | "abort" }` to end it by killing the worker instead (see below) — reach for `finish` when the conversation is genuinely complete and still open to a verdict, `stop` when you need a live process gone regardless.
