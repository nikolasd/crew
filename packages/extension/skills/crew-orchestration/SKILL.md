...
1. A finished turn parks the run at `waitingUser` and journals the turn-end — a TUI worker, which stays alive between turns, sits there. A protocol worker runs one turn and exits, so this park is transient by construction: the run you actually observe commonly already reads terminal (`cancelled`, ADR-0027's `unrendered_verdict()` — real work done, no `run/finish` call ever rendered a verdict on it). Either way, the `/crew` monitor emits a milestone for the settle the same way it does for terminal states, worker questions, and timeouts — you do not have to poll for it.
