...
only *that* the turn ended, and how.`,
        ...
        },
        {
          ...
          properties: {
            type: {
              type: "string",
              const: "adapterReconciliationEvent"
            },
            payload: {
              type: "object",
              properties: {
...
specific merge of org/repo/user/per-run layers.`,
    ...
    RunFlags: {
      ...
      properties: {
        ...
        turnSettled: {
          description: `True when the run reached \`waitingUser\` because its vendor finished
...
turn, not bookkeeping evidence.`,
    ...
    TurnOutcome: {
      description: `How a vendor's turn ended.
...
(\`hidden\`), not a placement.`,
      oneOf: [
        ...
        {
          description: `A new, separate OS-level window -- not a tab or split of the
...
persistence.`,
      ...
      properties: {
        ...
        payload: {
          description: `The message payload. Already redacted: secret-shaped substrings are
masked.`,
          $ref: "#/$defs/Redacted"
        },
        deliveryState: {
...
async function readCatalogue(adapter, run = defaultRun) {
  ...
  const ids = models.filter((m) => typeof m?.provider === "string" && typeof m?.id === "string").filter((m) => m.provider === provider).map((m) => m.id);
  if (ids.length === 0) {
...
function resolveModelName(adapter, input, catalogue) {
...
function resolutionNote(adapter, r) {
  switch (r.kind) {
    case "exact":
      return;
    case "alias":
...
function modelConflictResult(adapter, configuredModel) {
  return {
    content: [
      {
        type: "text",
...
class MilestoneTracker {
  ...
  hasSettledTurn(runId) {
    return this.#sawSettled.has(runId);
  }
}
function formatDigest(e, lookup, tracker) {
  const event = e.event;
