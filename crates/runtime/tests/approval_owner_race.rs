...
/// The journal sequence of the `approvalDecided` row for `approval_id`, if
/// the guarded write actually committed one. Diagnostic only, for the same
/// failure message as [`reconcile_event_sequence`] -- comparing the two
/// sequences (when both exist) tells a reader which write the actor
/// actually processed first, which is the fact the enqueue-order argument
/// in this file's header claims is fixed and this diagnostic exists to
/// confirm or refute on an actual recurrence rather than a rerun of the
/// same argument.
async fn decided_event_sequence(db: &DatabaseHandle, approval_id: ApprovalId) -> Option<i64> {
    db.run_domain_op(Box::new(move |conn| {
        let sequence: Option<i64> = conn
            .query_row(
...
async fn a_stale_owner_is_refused_by_the_guarded_write_after_a_rebind() {
    ...
    let decided_seq = decided_event_sequence(&db, approval_id).await;
