...
use crew_runtime::display::{
    CommandExecutor, CommandResult, DisplayBackendTrait, HerdrDisplay, PaneHandle, PaneRequest,
};
use std::io;
...
#[tokio::test]
async fn the_mismatch_fixture_makes_the_backend_unavailable_with_restart_guidance_and_issues_no_pane_command()
 {
    let executor = Arc::new(FixtureExecutor::new().with(
        "herdr status --json",
...
#[tokio::test]
async fn a_created_pane_updates_state_three_times_and_close_only_touches_crew_tagged_panes() {
    ...
    let handle = herdr
        .create_pane(pane_request(
            vec!["crewd", "monitor", "--run-id", "run-1"],
            "display-1",
        ))
        .await
        .expect("a compatible backend must create the pane");
    assert_eq!(handle.pane_ref, "w1:p9");
