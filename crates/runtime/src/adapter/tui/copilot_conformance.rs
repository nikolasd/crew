...
/// well-behaved CLI.
///
/// The second `echo` is load-bearing, not decorative: `CopilotTuiVendor`
/// now has a real `classify_surface` (`classify_copilot_surface`), which
/// looks for the composer's own footer chrome rather than treating any
/// output as ready -- the same reason codex's own fixture double needed
/// its composer banner added when that vendor's readiness poll stopped
/// accepting any output (see `codex_conformance.rs`'s own double). Without
/// this line every scenario using this double sits `Undecided` forever,
/// since the double's first (real, unchanged) banner alone no longer
/// reads as a recognized surface.
fn write_double(scripts_dir: &std::path::Path, session_dir: &std::path::Path) -> PathBuf {
    let script = format!(
        r#"#!/bin/sh
echo "Welcome to GitHub Copilot CLI!"
echo "<- open sidebar . / commands . ? help . tab next tab"
