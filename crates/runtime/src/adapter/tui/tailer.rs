...
impl TranscriptTailer {
    ...
    pub async fn poll_once(&mut self) -> Option<(Vec<TuiEvent>, Cursor)> {
        let raw = self.read_from_cursor().await?;
        if raw.is_empty() {
            return None;
        }
        let (events, new_cursor) = self.format.parse(&raw, &self.cursor);
        if new_cursor.offset == self.cursor.offset {
            return None;
        }
        self.cursor = new_cursor.clone();
        Some((events, new_cursor))
    }
    ...
    pub fn spawn(
        mut self,
        mut on_batch: impl FnMut(Vec<TuiEvent>, Cursor) + Send + 'static,
    ) -> TailerHandle {
        let (stop_tx, mut stop_rx) = watch::channel(false);
        let task = tokio::spawn(async move {
            loop {
                if let Some((events, cursor)) = self.poll_once().await {
