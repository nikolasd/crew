...
impl<'c> DomainRepository<'c> {
    ...
    // `settle_abandoned_turn` (ADR-0027 wave 3's inactivity backstop,
    // which settled a leader-abandoned turn to `lost` after inactivity
    // alone) is removed: the maintainer ruled that silence is never
    // evidence a leader gave up, only its connection actually being gone
    // is. See ADR-0036 and `crate::ipc::leader_registry`.
